...
export const checkAndGenerateCertificate = async (userId, courseId) => {
    try {
        ...
        const [watchedCountResult] = await db.select({ count: count() })
            .from(userModuleProgress)
            .where(and(eq(userModuleProgress.userId, userId), eq(userModuleProgress.videoWatched, true), inArray(userModuleProgress.moduleId, moduleIds)));
        ...
        if (quizCount > 0) {
            const passedQuizzesResult = await db.select({ quizId: quizAttempts.quizId })
                .from(quizAttempts)
                .where(and(eq(quizAttempts.userId, userId), eq(quizAttempts.passed, true), inArray(quizAttempts.quizId, quizIds)));
...
export const manualGenerateCertificate = async (req, res) => {
